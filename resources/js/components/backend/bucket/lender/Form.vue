        ...
        <form @submit.prevent="submitForm"
            ...
            <div class="row">
                <div class="col-sm-6 form-group">
                    <label for="bucket_name"><b>Input risk bucket name <small v-if="method != 'put'"> (Parent name: {{ value.name }})</small></b></label>
...
<script>
    ...
    export default {
        ...
        computed: {
            minAndMaxRule() {
                if (_.isEmpty(this.value)) return {required: true}
                return {
                    required: true,
                    min_value: this.value.min_score,
                    max_value: this.value.max_score
                }
            }
        },
        data (){
        ...
        methods: {
            ...
            storeBucket (){
                ...
                this.$validator.validateAll().then((result) => {
                    if (result) {
                        self.load = true;
                        client.post(route + '/'+ self.value.id +'/store', self.bucket)
                            .then(response => {
                                self.load = false;
